...
class MainMenu(cmd.Cmd):
    ...
    def __init__(self, args=None):
        ...
        self.prompt = '(Invader)> '
        self.do_help.__func__.__doc__ = '''Display the help menu.'''
    ...
    def handle_args(self):
        ...
        """
	if self.args.run:
	    resourceFile = self.args.run[0]
	    self.do_iscript(resourceFile)
    ...
    def cmdloop(self):
        ...
        while True:
            ...
            # handle those pesky ctrl+c's
                ...
                self.menu_state = "Main"
                try:
                    choice = raw_input(helpers.color("[?] Exit Invader Framework? [y/N] ", "red"))
    ...
    def do_iscript(self, arg):
	"Run an InvaderScript program."
	...
	import sys
        ...
        if (os.path.exists("/data/data/com.termux")):
            ispath = "/data/data/com.termux/files/usr/bin/iscript"
        else:
            ispath = "/usr/local/bin/iscript"
            
        if (os.path.exists(ispath)):
            if True:
                if (os.path.exists("/tmp/invader")):
                    time.sleep(0)
        	else:
                    os.system("mkdir /tmp/invader")
                   
        	if (os.path.exists("/tmp/invader/inpath.temp")):
          	    os.system("rm /tmp/invader/inpath.temp")
          	    os.system("echo $OLDPWD >> /tmp/invader/inpath.temp")
        	else:
           	    os.system("echo $OLDPWD >> /tmp/invader/inpath.temp")
                    
        	if not '/' in resourceFile:
          	    run = open("/tmp/invader/inpath.temp").read().split('\n')[-2]+'/'+resourceFile
	  	    os.system("rm -r /tmp/invader")
        	else:
          	    run = resourceFile 
	   	    os.system("rm -r /tmp/invader")  
                ...
                if (os.path.exists(run)):
                    ...
                    WHSL="\033[0;97m"
                    ENDL="\033[0m"
                    print(REDL+"[!] No such InvaderScript program file"+ENDL)
                    import sys
                    sys.exit()
                ...
                if '.' in argrem:
                    ...
                    WHSL="\033[0;97m"
                    ENDL="\033[0m"
                    print(REDL+"[!] InvaderScript program file is not executable"+ENDL)
                    import sys
                    sys.exit()
                
                try:
                    if (argrem[-(argrem[::-1].index('.')):]) == 'isc':
                        name = argrem[:argrem.index('.')]
                        os.system("python3 script/runner/runner.py -r "+argrem)
                        if (os.path.exists("/tmp/iscript")):
                            if (os.path.exists("/tmp/iscript/program.i")):
                                os.system("rm /tmp/iscript/program.i")
                        else:
                            os.system("mkdir /tmp/iscript")
                    else:
                        REDL="\033[1;31m"
                        WHSL="\033[0;97m"
                        ENDL="\033[0m"
                    ...
                    os.system("mv "+name+".i /tmp/iscript/program.i")
                except:
                    import sys
                    sys.exit()  
            ...
            WHSL="\033[0;97m"
            ENDL="\033[0m"
            print(REDL+"[!] InvaderScript is not installed"+ENDL)
            ips = raw_input('Install InvaderScript [y/N] ')
            if ips == "y" or ips == "Y":
    ...
    def do_exit(self, line):
        "Exit from the Invader Framework."
    ...
    def do_delay(self, arg):
        "Invader Framwork delay in seconds."
        import time
        time.sleep(int(arg))
...
class AgentsMenu(SubMenu):
    ...
    def do_clean(self, line):
        "Clean one or more agent's taskings."
...
class PowerShellAgentMenu(SubMenu):
    ...
    def __init__(self, mainMenu, sessionID):
        ...
        self.doc_header = 'Windows Commands'
    ...
    def do_help(self, *args):
        "Display the help menu or syntax for particular commands."
...
class PythonAgentMenu(SubMenu):
    ...
    def __init__(self, mainMenu, sessionID):
        ...
        self.doc_header = 'Linux Commands'
